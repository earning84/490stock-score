...
[4단계: 5대 영역 심층 리포트(memberReport) 및 keyPoint 작성 (매우 중요)]
- keyPoint: 3줄 이내 요약
- memberReport: 회원 모드 전용 5대 핵심 영역 심층 분석 리포트를 아래 규칙에 맞추어 반드시 충실하게 작성하십시오.
  * 영역별 구성:
    1. megaTrend: 메가트렌드 분석 (항목 1번부터 9번까지의 내용을 통합 요약)
    2. moat: 해자 및 경쟁력 분석 (항목 10번부터 20번까지의 내용을 통합 요약)
    3. financial: 재무 및 수익성 분석 (항목 21번부터 37번까지의 내용을 통합 요약)
    4. valuation: 밸류에이션 분석 (항목 38번부터 44번까지의 내용을 통합 요약)
    5. investmentOpinion: 종합 투자 의견 (항목 7번과 17번을 재요약하고, 투자 적격 조건 충족 여부에 따라 실제 투자 결정을 어떻게 바라보아야 할지 매우 '조심스럽게' 최종 제안)
  * 분량 기준:
    - 한국어: 하나의 영역당 반드시 1,000자 내외로 풍부하고 정밀하게 서술하십시오.
    - 영어: 하나의 영역당 반드시 2,000자 내외로 서술하십시오.
  * 작성 규칙 (절대 엄수):
    ...
    4) 글 배치 및 흐름: 인기 사주/운세 서비스처럼 모바일에서 술술 읽히는 자연스러운 스토리텔링 흐름을 만드십시오.
    5) 모바일 줄바꿈: 대부분의 독자가 모바일로 글을 읽으므로, 간결한 문장 단위로 줄바꿈(\\n)을 자주 해 주십시오.
    6) 단락 여백: 3~4문장이 모여 한 단락이 끝나면 반드시 빈 줄 2줄 이상(\\n\\n\\n)을 넣어 시각적 답답함을 완전히 없애십시오.
...
반환 JSON 규격 (상장 기업인 경우):
{
  ...
  "memberReport": {
    "megaTrend": "메가트렌드 심층 분석 내용...",
    "moat": "해자 및 경쟁력 심층 분석 내용...",
    "financial": "재무 및 수익성 심층 분석 내용...",
    "valuation": "밸류에이션 심층 분석 내용...",
